...
class WandBLogger(BaseLogger, BasePlugin):
    ...
    def before_run(self):
        ...
        self.wandb.run._label(repo="Avalanche-RL")
    ...
    def log_single_metric(self, name, value, x_plot):
        ...
        if not isinstance(
            ...
            # Unsupported type
            print(f">>> Unsupported: {name} {value} {type(value)} <<<")
            return
        ...
        if isinstance(value, Image):
            ...
            # print(f">> step({self.step}) {name} : {value} <<")
            self.wandb.log({
                name : value, 
                "plot_step" if "train" in name else "eval_step" : self.step
            })
